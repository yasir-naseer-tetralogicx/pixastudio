    ...
    <div class="row mt-5">
      <div class="col-md-6">
	      ...
	      @if($position)
	        ...
	        <p>
		 Width: {{ $position->crop_width }} px <br>
              	 Hieght: {{ $position->crop_height }} px<br>
		 Top Left x cordinate: {{ $position->top_left_x}} px<br>
		 Top Left y cordinate: {{ $position->top_left_y }} px<br>
                 Bottom Right x cordinate: {{ $position->bottom_right_x}} px<br>
		 Bottom Right y cordinate: {{ $position->bottom_right_y }} px<br>
      ...
      <div class="col-md-6">
          ...
          <img src="{{  $product['image']['src'] }}" alt="No Image" id="img" class="d-none">
          <canvas id="canvas" ></canvas>
          <p class="text-primary">Note: Please draw on the image to select the area you want to place your design</p>

          
          <form action="{{ route('upload.crop', $product['id']) }}" method="POST">
            @csrf
